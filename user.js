...
(function () {
  ...
  function logout() {
    ...
    (function() {
      ...
      _lo.innerHTML = '<div style="width:42px;height:42px;border:3px solid rgba(37, 99, 235,0.3);border-top-color:#2563EB;border-radius:50%;animation:spin .7s linear infinite;"></div><div style="color:#2563EB;font-family:sans-serif;font-size:14px;font-weight:600;letter-spacing:.5px;">Logging out…</div>';
  ...
  function openMyProfile() {
    ...
    let fb = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='88' height='88'><circle cx='44' cy='44' r='44' fill='%232563EB'/><text x='44' y='56' text-anchor='middle' fill='white' font-size='36' font-family='Arial'>&#128100;</text></svg>";
    let st = String(myProfile.Status || "Active");
    let stC = st.toLowerCase() === "active" ? "#22c55e" : st.toLowerCase() === "pending" ? "#f59e0b" : "#ef4444";
    let html = `
        <div class="_mhdr" style="background:linear-gradient(135deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);border-bottom:2px solid rgba(37, 99, 235,0.35);"><h3 style="color:#fff;display:flex;align-items:center;gap:8px;"><span class="_u-icon-md"><i class="fa-solid fa-id-card" style="color:#2563EB;font-size:13px;"></i></span> My Profile</h3><button class="_mcls" onclick="closeModal()" style="color:rgba(255,255,255,0.6);font-size:20px;line-height:1;background:none;border:none;cursor:pointer;padding:0;">×</button></div>
        <div class="_mbdy" style="padding:0;">
          <div style="background:linear-gradient(180deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);padding:26px 20px 20px;text-align:center;position:relative;overflow:hidden;">
            <div style="position:absolute;top:-40px;right:-40px;width:160px;height:160px;background:radial-gradient(circle,rgba(37, 99, 235,0.15),transparent 70%);border-radius:50%;pointer-events:none;"></div>
            <div style="position:absolute;bottom:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(37, 99, 235,0.4),transparent);"></div>
            <img id="_profileModalPhoto" src="${escapeHtml(photoSrc)}" onerror="this.src='${fb}'"
              style="width:82px;height:82px;border-radius:50%;object-fit:cover;border:3px solid #2563EB;background:#eee;display:block;margin:0 auto 10px;box-shadow:0 4px 20px rgba(37, 99, 235,0.45);"/>
            <div style="font-family:'Sora',sans-serif;color:#2563EB;font-size:1.05rem;font-weight:700;margin-bottom:8px;">${escapeHtml(myProfile.Name || "—")}</div>
          ...
          <div style="padding:10px 20px 16px;background:var(--white);">
            <div class="_row" style="border-bottom:1px solid rgba(37, 99, 235,0.12);display:flex;align-items:center;justify-content:space-between;gap:8px;"><span class="_rl" class="_u-meta-ns"><span class="_u-icon-sm"><i class="fa-solid fa-mobile-screen" class="_u-gold-icon"></i></span> Mobile</span><span class="_rv" style="font-weight:600;color:#1e293b;">${escapeHtml(String(myProfile.Mobile || "—"))}</span></div>
            ...
            <div class="_mft" style="gap:8px;border-top:2px solid rgba(37, 99, 235,0.15);background:linear-gradient(90deg,rgba(37, 99, 235,0.04),transparent);">
    ...
    <button class="_mbtn" style="background:linear-gradient(135deg,#2a0f00,#3c1a00);box-shadow:0 3px 10px rgba(42,15,0,0.3);" onclick="closeModal();openChangePassword()">
      <i class="fa-solid fa-key" style="color:#2563EB;"></i> Change Password
    </button>
    <button class="_mbtn" style="background:linear-gradient(135deg,#2563EB,#e8920a);box-shadow:0 3px 10px rgba(37, 99, 235,0.35);" onclick="closeModal();openEditProfile()">
  ...
  function toggleProfilePassword() {
    ...
    if (!val || !btn) return;
    if (btn.textContent === "Show") { val.textContent = window._myProfilePwdHash || "(not available)"; val.style.color = "#334155"; val.style.letterSpacing = "0.5px"; btn.textContent = "Hide"; btn.style.background = "#fff3e0"; btn.style.borderColor = "#2563EB"; btn.style.color = "#c0580a"; }
  ...
  function openEditProfile(previewB64, prefillName, prefillEmail, prefillVillage, prefillAddress, prefillDob) {
    ...
    if (rawDob) {
      if (/^\d{2}-\d{2}-\d{4}$/.test(rawDob)) {
        ...
        const _dp = rawDob.split("T")[0]; if (/^\d{4}-\d{2}-\d{2}$/.test(_dp)) dDob = _dp;
      }
    }
    ...
    let fb = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' width='88' height='88'><circle cx='44' cy='44' r='44' fill='%232563EB'/><text x='44' y='56' text-anchor='middle' fill='white' font-size='36' font-family='Arial'>&#128100;</text></svg>";
    let html = `
        <div class="_mhdr" style="background:linear-gradient(135deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);border-bottom:2px solid rgba(37, 99, 235,0.35);"><h3 style="color:#fff;display:flex;align-items:center;gap:8px;"><span class="_u-icon-md"><i class="fa-solid fa-user-pen" style="color:#2563EB;font-size:13px;"></i></span> Edit Profile</h3><button class="_mcls" onclick="closeModal()" style="color:rgba(255,255,255,0.6);font-size:20px;line-height:1;background:none;border:none;cursor:pointer;padding:0;">×</button></div>
        <div class="_mbdy" style="padding:0;">
          ...
          <div style="background:linear-gradient(180deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);padding:22px 20px 18px;text-align:center;position:relative;overflow:hidden;">
            ...
            <div style="position:relative;width:82px;margin:0 auto 8px;">
              <img id="photoPreview" src="${escapeHtml(photoSrc)}" onerror="this.src='${fb}'"
                style="width:82px;height:82px;border-radius:50%;object-fit:cover;border:3px solid #2563EB;background:#eee;display:block;box-shadow:0 4px 20px rgba(37, 99, 235,0.45);"/>
              <div onclick="pickPhoto()" title="Change Photo"
                style="position:absolute;bottom:2px;right:2px;width:26px;height:26px;background:#2563EB;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.25);border:2px solid #fff;">
                <i class="fa-solid fa-camera" style="color:#fff;font-size:10px;"></i>
              </div>
            </div>
            <div style="font-size:11px;color:rgba(37, 99, 235,0.65);margin:0;">Tap camera to change photo</div>
          </div>
          ...
          <div style="padding:6px 20px 14px;background:var(--white);">
            ...
            <!-- Full Name -->
            <div class="_row" style="border-bottom:1px solid rgba(37, 99, 235,0.12);align-items:center;">
            ...
            <!-- Address -->
            <div class="_row" style="border-bottom:1px solid rgba(37, 99, 235,0.12);align-items:flex-start;padding-top:11px;padding-bottom:11px;">
            ...
            <!-- Date of Birth -->
            <div class="_row" style="border-bottom:1px solid rgba(37, 99, 235,0.12);align-items:center;position:relative;overflow:hidden;">
              <span class="_rl" class="_u-meta">
                <span class="_u-icon-sm">
              ...
              <div style="display:flex;align-items:center;gap:6px;justify-content:flex-end;flex:1;pointer-events:none;">
                ...
                <i class="fa-solid fa-calendar-days" style="color:var(--gold,#2563EB);font-size:13px;flex-shrink:0;"></i>
              </div>
        ...
        <div class="_mft" style="border-top:2px solid rgba(37, 99, 235,0.15);background:linear-gradient(90deg,rgba(37, 99, 235,0.04),transparent);">
          <button class="_mbtn" style="background:#64748b;box-shadow:none;" onclick="closeModal();_pendingCroppedB64='';">Cancel</button>
          <button class="_mbtn" style="background:linear-gradient(135deg,#2563EB,#e8920a);box-shadow:0 3px 10px rgba(37, 99, 235,0.35);" onclick="saveProfile()"><i class="fa-solid fa-check"></i> Save Changes</button>
        </div>`;
    openModal(html, "min(460px, 96vw)");
  ...
  function openUpdateMobileConfirm() {
    ...
    overlay.innerHTML = `
        <div style="background:var(--white);border-radius:18px;max-width:380px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);overflow:hidden;animation:dropFade .2s ease;">
          ...
          <div style="background:linear-gradient(135deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);padding:16px 20px;display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid rgba(37, 99, 235,0.35);">
            <div style="display:flex;align-items:center;gap:10px;">
              <span style="width:32px;height:32px;background:rgba(37, 99, 235,0.18);border-radius:9px;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;">
                <i class="fa-solid fa-mobile-screen-button" style="color:#2563EB;font-size:14px;"></i>
              </span>
          ...
          <div style="padding:20px 20px 16px;">
            ...
            <div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:16px;background:var(--bg);border:1.5px solid rgba(37, 99, 235,0.22);border-radius:10px;padding:12px 14px;">
              <div style="width:36px;height:36px;background:rgba(37, 99, 235,0.15);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">
            ...
            <div style="background:var(--bg);border:1.5px solid rgba(37, 99, 235,0.22);border-radius:9px;padding:10px 14px;display:flex;align-items:center;gap:10px;margin-bottom:16px;">
              <i class="fa-solid fa-mobile-screen" style="color:#2563EB;font-size:14px;"></i>
            ...
            <div id="_mobileStep1">
              ...
              <div style="display:flex;gap:8px;">
                ...
                <button onclick="_mobileGoToStep2()" style="flex:2;padding:10px;background:linear-gradient(135deg,#2563EB,#e8920a);color:#fff;border:none;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 10px rgba(37, 99, 235,0.35);display:inline-flex;align-items:center;justify-content:center;gap:7px;">
            ...
            <div id="_mobileStep2" style="display:none;">
              ...
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:14px;">
                <span style="width:22px;height:22px;background:linear-gradient(135deg,#2563EB,#e8920a);border-radius:50%;color:#fff;font-size:11px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;">1</span>
                <span style="font-size:11.5px;font-weight:600;color:#2563EB;">Enter new number</span>
                <span style="flex:1;height:1.5px;background:linear-gradient(90deg,rgba(37, 99, 235,0.4),transparent);"></span>
              ...
              <button id="_mobileSendOtpBtn" onclick="_mobileSendOTP()" style="width:100%;padding:11px;background:linear-gradient(135deg,#2563EB,#e8920a);color:#fff;border:none;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;box-shadow:0 3px 10px rgba(37, 99, 235,0.35);display:flex;align-items:center;justify-content:center;gap:8px;box-sizing:border-box;margin-bottom:8px;">
            ...
            <div id="_mobileStep3" style="display:none;">
              ...
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:14px;">
                ...
                <span style="flex:1;height:1.5px;background:linear-gradient(90deg,rgba(37, 99, 235,0.4),transparent);"></span>
                <span style="width:22px;height:22px;background:linear-gradient(135deg,#2563EB,#e8920a);border-radius:50%;color:#fff;font-size:11px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;">2</span>
                <span style="font-size:11.5px;font-weight:600;color:#2563EB;">Verify OTP</span>
              </div>
              ...
              <div style="text-align:center;margin-bottom:4px;">
                ...
                <button id="_mobileResendBtn" onclick="_mobileResendOTP()" style="display:none;padding:5px 14px;background:none;color:#2563EB;border:1.5px solid #2563EB;border-radius:7px;font-size:12px;font-weight:700;cursor:pointer;box-shadow:none;transition:background .2s;">
...
═══════════════════════════════════════════════════════════════ */
  function openChangePassword() {
    const html = `
  <div class="_mhdr" style="background:linear-gradient(135deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);border-bottom:2px solid rgba(37, 99, 235,0.35);">
    <h3 style="color:#fff;display:flex;align-items:center;gap:8px;"><span class="_u-icon-md"><i class="fa-solid fa-key" style="color:#2563EB;font-size:13px;"></i></span> Change Password</h3>
  ...
  <div class="_mft" style="border-top:2px solid rgba(37, 99, 235,0.15);background:linear-gradient(90deg,rgba(37, 99, 235,0.04),transparent);">
    ...
    <button class="_mbtn" id="cp_save_btn" style="background:linear-gradient(135deg,#2563EB,#e8920a);box-shadow:0 3px 10px rgba(37, 99, 235,0.35);"
  ...
  function _showForceChangePassword(session) {
    ...
    ov.innerHTML = [
      ...
      '#forcePwdCard{background:linear-gradient(160deg,#2d1a00,#1e1000);border:1.5px solid rgba(37, 99, 235,0.35);border-radius:24px;padding:32px 28px 28px;max-width:400px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.6);animation:slideUp .4s cubic-bezier(.22,1,.36,1);}',
      '#forcePwdCard input{width:100%;box-sizing:border-box;background:rgba(255,255,255,0.07);border:1.5px solid rgba(37, 99, 235,0.25);border-radius:12px;color:#f1e8d8;font-size:15px;padding:12px 44px 12px 14px;outline:none;transition:border .2s;}',
      '#forcePwdCard input:focus{border-color:rgba(37, 99, 235,0.7);}',
      '#forcePwdCard input::placeholder{color:rgba(241,232,216,0.35);}',
      '.fpwd-eye{position:absolute;right:13px;top:50%;transform:translateY(-50%);cursor:pointer;color:rgba(37, 99, 235,0.6);font-size:14px;}',
      '.fpwd-eye:hover{color:#2563EB;}',
      '#fpwd_strength_bar{height:5px;border-radius:3px;transition:width .3s,background .3s;width:0%;}',
      '#fpwd_save_btn{width:100%;padding:14px;border:none;border-radius:14px;font-size:15px;font-weight:700;cursor:pointer;background:linear-gradient(135deg,#2563EB,#e8920a);color:#1a0800;box-shadow:0 4px 16px rgba(37, 99, 235,0.35);transition:opacity .2s;letter-spacing:.3px;}',
      ...
      '<div id="forcePwdCard">',
        ...
        '<div style="text-align:center;margin-bottom:24px;">',
          '<div style="width:56px;height:56px;background:rgba(37, 99, 235,0.15);border:2px solid rgba(37, 99, 235,0.4);border-radius:18px;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;">',
            '<i class="fa-solid fa-shield-halved" style="color:#2563EB;font-size:24px;"></i>',
          '</div>',
          '<div style="color:#2563EB;font-size:20px;font-weight:700;margin-bottom:6px;">Set Your Password</div>',
  ...
  function renderGoals(goals) {
    ...
    c.innerHTML = `<div class="sec-lbl"><i class="fa-solid fa-bullseye"></i> Mandir Goals</div>` +
      enabled.map(g => {
        ...
        let bc = pct >= 100 ? "#22c55e" : pct >= 60 ? "#2563EB" : "#ef4444";
        let sp = pct >= 100 ? `<span class="goal-pill" style="background:rgba(34,197,94,0.13);color:#16a34a;">✓ Achieved</span>` : `<span class="goal-pill" style="background:rgba(37, 99, 235,0.12);color:#2563EB;">${pct}% funded</span>`;
  ...
  async function renderBroadcasts() {
    ...
    const pC = { urgent: "#ef4444", important: "#2563EB", normal: "#334155" };
  ...
  function _renderRecentActivity() {
    ...
    el.innerHTML = recent.map(function(c) {
      ...
      var tipHtml = tipRows.map(function(r) {
        return '<div style="display:flex;justify-content:space-between;gap:12px;padding:3px 0;border-bottom:1px solid rgba(37, 99, 235,0.08);">'
      ...
      return '<div class="ra-item ra-item-noclk" style="cursor:default;position:relative;">'
        ...
        + '<div style="font-family:var(--font-h);font-size:11px;font-weight:700;color:var(--gold);margin-bottom:6px;padding-bottom:5px;border-bottom:1.5px solid rgba(37, 99, 235,0.2);">Transaction Details</div>'
        + tipHtml
        + '</div>'
        + '</div>';
    }).join("");
  ...
  function _raItemClick(el) {
    ...
    var rowsHtml = Array.from(rows).map(function(r) {
      ...
      return '<div style="display:flex;justify-content:space-between;align-items:center;padding:9px 0;border-bottom:1px solid rgba(37, 99, 235,0.1);">'
    ...
    overlay.innerHTML = '<div style="background:' + bgCard + ';width:100%;max-width:480px;border-radius:20px 20px 0 0;padding:0 0 20px;box-shadow:0 -8px 40px rgba(0,0,0,0.3);animation:slideUpSheet .22s ease;">'
      + '<div style="width:40px;height:4px;background:rgba(37, 99, 235,0.35);border-radius:2px;margin:10px auto 0;"></div>'
      + '<div style="background:linear-gradient(135deg,#2a0f00,#3c1a00);padding:13px 16px 11px;margin:8px 0 0;display:flex;align-items:center;justify-content:space-between;border-bottom:2px solid rgba(37, 99, 235,0.3);">'
      + '<div style="display:flex;align-items:center;gap:9px;">'
      + '<div style="width:30px;height:30px;background:rgba(37, 99, 235,0.18);border-radius:8px;display:flex;align-items:center;justify-content:center;">'
      + '<i class="fa-solid fa-receipt" style="color:#2563EB;font-size:0.8rem;"></i></div>'
  ...
  async function exportPDF() {
    ...
    if (rows.length > 0) {
      doc.autoTable({
        ...
        body: rows, startY: Y,
        theme: "grid",
        headStyles: { fillColor: [42, 15, 0], textColor: [37, 99, 235], fontStyle: "bold", fontSize: 9 },
...
═══════════════════════════════════════════════════════════════ */
  ...
  function _loadUserEvents() {
    ...
    getData("getEventData").then(function (res) {
      ...
      const catColors = { Festival: "#2563EB", Pooja: "#8b5cf6", Maintenance: "#64748b", Community: "#22c55e", Other: "#3b82f6" };
  ...
  function _confirmPdfDownload(type) {
    ...
    const borderC = isDark ? "rgba(37, 99, 235,0.22)" : "rgba(37, 99, 235,0.18)";
    ...
    ov.innerHTML = `
      <div style="background:${bgCard};border-radius:20px;max-width:370px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.45);border:1.5px solid ${borderC};overflow:hidden;animation:dropFade .2s ease;">
        ...
        <!-- Header -->
        <div style="background:linear-gradient(135deg,#2a0f00 0%,#3c1a00 60%,#2a0f00 100%);padding:16px 18px;display:flex;align-items:center;gap:11px;border-bottom:2px solid rgba(37, 99, 235,0.35);">
          <div style="width:38px;height:38px;background:rgba(37, 99, 235,0.18);border-radius:11px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <i class="fa-solid ${iconClass}" style="color:#2563EB;font-size:1rem;"></i>
          </div>
          <div class="_u-flex-1">
            <div style="font-weight:700;font-size:13.5px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${title}</div>
            <div style="font-size:10px;color:rgba(37, 99, 235,0.65);margin-top:1px;">Download confirmation</div>
          </div>
        ...
        <div style="padding:20px 18px 6px;text-align:center;">
          <div style="width:52px;height:52px;background:rgba(37, 99, 235,0.12);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
            <i class="fa-solid fa-file-arrow-down" style="color:#2563EB;font-size:1.4rem;"></i>
          </div>
          ...
          <div style="display:inline-flex;align-items:center;gap:5px;margin-top:8px;background:rgba(37, 99, 235,0.08);border:1px solid rgba(37, 99, 235,0.18);border-radius:8px;padding:5px 11px;">
        ...
        <div style="padding:18px 18px 22px;display:flex;gap:10px;justify-content:center;">
          ...
          <button id="_pdfConfirmBtn" onclick="_doPdfDownload('${type}')"
            style="min-width:140px;padding:12px 20px;background:linear-gradient(135deg,#2563EB,#e8920a);color:#fff;border:none;border-radius:11px;font-family:var(--font-b);font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 14px rgba(37, 99, 235,0.38);display:inline-flex;align-items:center;justify-content:center;gap:6px;">
...
if (isDark) {
    ...
    try {
      ...
      listEl.innerHTML = visible.map(function (r) {
        ...
        if (isPending) {
          actionBtns = `
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;">
                ...
                <button class="btn-sm req-edit-btn" onclick="_editBtnClick(this,'${reqId}')" style="font-size:11px;padding:6px 14px;border-radius:20px;background:linear-gradient(135deg,#2a0f00,#3c1a00);color:#2563EB;border:1.5px solid rgba(37, 99, 235,0.35);font-weight:700;box-shadow:0 2px 8px rgba(42,15,0,0.2);transition:all 0.18s;"><i class="fa-solid fa-pen-to-square"></i> Edit Request</button>
              </div>`;
        } else {
          actionBtns = "";
        }
        ...
        return `<div class="card" style="margin-bottom:10px;border-left:4px solid ${stColor};">
            <div class="card-body" style="padding:12px 16px;">
              ...
              ${isPending ? `<div style="margin-top:6px;font-size:11.5px;color:#92400e;background:rgba(37, 99, 235,0.1);padding:6px 10px;border-radius:6px;">Admin will review and record this payment soon.</div>` : ""}
      ...
      listEl.innerHTML = `<div class="card" style="margin-bottom:10px;">
        <div class="card-body" style="text-align:center;padding:22px 16px;">
          ...
          <button onclick="_contribReqsLoaded=false;_loadUserContribRequests();" style="background:linear-gradient(135deg,#2563EB,#e8920a);color:#fff;border:none;border-radius:20px;padding:7px 18px;font-size:12px;font-weight:700;cursor:pointer;">
  ...
  function previewContribSlip(url) {
    ...
    m.innerHTML = `<div style="background:var(--white);border-radius:16px;max-width:540px;width:100%;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,0.45);">
        <div style="padding:13px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(37, 99, 235,0.15);background:var(--bg);">
  ...
  async function openEditContribRequest(reqId) {
    ...
    m.innerHTML = `<div style="background:var(--white);border-radius:16px;max-width:480px;width:100%;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.35);">
        <div style="padding:14px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(37, 99, 235,0.15);background:var(--bg);">
  ...
  function _showBirthdayCelebration(userName, bdayKey) {
    ...
    overlay.innerHTML = [
      ...
      '@keyframes _bdayGlow{0%,100%{box-shadow:0 0 35px rgba(37, 99, 235,0.3),0 20px 60px rgba(0,0,0,0.8)}50%{box-shadow:0 0 70px rgba(37, 99, 235,0.55),0 20px 60px rgba(0,0,0,0.8)}}',
      ...
      /* ── Card ── */
      '#_bdayCelebCard{position:relative;z-index:2;background:linear-gradient(170deg,#3d1a00 0%,#260e00 55%,#160700 100%);border:1.5px solid rgba(37, 99, 235,0.5);border-radius:26px;padding:28px 24px 22px;max-width:370px;width:100%;text-align:center;animation:_bdayCardIn 0.65s cubic-bezier(.22,1,.36,1) 0.1s both,_bdayGlow 3s ease 0.8s infinite;flex-shrink:0;}',
      /* ── Shine sweep ── */
      '#_bdayCelebCard::after{content:"";position:absolute;top:0;left:-120%;width:55%;height:100%;background:linear-gradient(90deg,transparent,rgba(37, 99, 235,0.06),transparent);border-radius:26px;animation:_bdayShine 4s ease 1.2s infinite;pointer-events:none;}',
      /* ── Content styles ── */
      '._bday-om{font-size:1rem;color:rgba(37, 99, 235,0.65);letter-spacing:8px;margin-bottom:8px;display:block;}',
      '._bday-divider{width:90px;height:1px;background:linear-gradient(90deg,transparent,rgba(37, 99, 235,0.55),transparent);margin:8px auto;}',
      ...
      '._bday-title{font-family:\'Sora\',sans-serif;font-size:1.5rem;font-weight:800;background:linear-gradient(135deg,#2563EB 0%,#93C5FD 50%,#2563EB 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.25;margin:10px 0 2px;padding:0 4px;}',
      '._bday-sub{font-size:0.72rem;font-weight:700;color:rgba(37, 99, 235,0.6);letter-spacing:2.5px;margin-bottom:10px;}',
      '._bday-name{font-family:\'Sora\',sans-serif;font-size:1.1rem;font-weight:700;color:#fff;margin-bottom:3px;}',
      '._bday-sanskrit{font-size:10.5px;color:rgba(37, 99, 235,0.6);letter-spacing:0.8px;margin-bottom:10px;font-style:italic;line-height:1.5;}',
      '._bday-msg{font-size:12.5px;color:rgba(255,255,255,0.8);line-height:1.78;margin-bottom:5px;padding:0 2px;}',
      '._bday-msg2{font-size:11.5px;color:rgba(37, 99, 235,0.72);line-height:1.65;margin-bottom:18px;padding:0 2px;}',
      '._bday-btn{background:linear-gradient(135deg,#2563EB,#e8920a);color:#fff;border:none;border-radius:50px;padding:13px 40px;font-family:\'DM Sans\',sans-serif;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 22px rgba(37, 99, 235,0.55);transition:transform 0.15s;}',
      ...
      '._bday-timer-fill{height:100%;background:linear-gradient(90deg,#2563EB,#93C5FD);border-radius:3px;animation:_bdayTimerShrink 18s linear forwards;}',
      ...
      /* Glowing orbs */
      '<div class="_bday-orb" style="width:320px;height:320px;top:-80px;left:-80px;background:radial-gradient(circle,rgba(37, 99, 235,0.22),transparent 70%);animation-duration:5s;"></div>',
      ...
      '<div class="_bday-orb" style="width:180px;height:180px;top:40%;left:60%;background:radial-gradient(circle,rgba(37, 99, 235,0.16),transparent 70%);animation-duration:6s;animation-delay:2s;"></div>',
    ...
    (function() {
      ...
      var particles = [];
      for (var j = 0; j < 25; j++) {
        particles.push({ x: Math.random() * 1, y: Math.random() * 1, vy: -(Math.random() * 0.0008 + 0.0003), r: Math.random() * 3 + 1.5, alpha: Math.random() * 0.5 + 0.3, color: Math.random() > 0.5 ? [37,99,235] : [143,185,168] });
      }
    ...
    var _petalEmoji = ["🌸","🌺","🌼","🪷","🌹","✨","🌻"];
    var _sparkRgb = [[37,99,235],[143,185,168],[251,191,36],[255,255,255],[251,146,60]];
